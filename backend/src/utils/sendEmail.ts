...
export const sendPasswordResetEmail = async (email: string, token: string, passcode: string) => {
  ...
  try {
    ...
    if (error instanceof Error) {
      logger.error('Failed to send password reset email', { email, error: error.message, stack: error.stack });
    } else {
