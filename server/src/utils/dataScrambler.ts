...
export async function updateMovementsData() {
    try {
      ...
      while (true) {
        ...
        const movements: IMovement[] = await Movement.find().skip(skipCount).limit(batchSize);
        if (movements.length === 0) break; // Exit loop if no more documents

        const bulkOps = [];
        for (const movementDoc of movements) {
          const client = await Client.findOne({ CODICE: movementDoc["Codice Cliente"] });
          ...
          (["Valore", "Costo", "Prezzo Articolo"] as const).forEach((field) => {
            if (typeof movementDoc[field] === "number") {
              updatedFields[field] = Number((movementDoc[field] * (1 + randomPercentage)).toFixed(2));
            }
          });
        ...
        const progressPercentage = ((processedCount / totalMovements) * 100).toFixed(2);
