...
import { useGetMinimalClientsQuery, useGetMinimalAgentsQuery, useGetClientByIdQuery, useGetAgentsQuery } from '../../services/api';
import { AuthHandlersProps } from '../../models/models';

const useAuthHandlers = ({ selectedRole, selectedAgent, selectedClient, agents }: AuthHandlersProps) => {
  ...
  const { data: fullClient, refetch: refetchClientById } = useGetClientByIdQuery(selectedClient, { skip: !selectedClient });
  ...
  }, [selectedRole, selectedAgent, selectedClient, agents, fullClient, dispatch, refetchClientById]);
