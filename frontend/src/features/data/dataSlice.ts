...
export const fetchData = createAsyncThunk<FetchDataPayload>('data/fetchData', async () => {
...
const dataSlice = createSlice({
  ...
  reducers: {
    setAgentClients(state, action: PayloadAction<Client[]>) {
      ...
      state.clientIndex = new Map<string, Client>(action.payload.map((client) => [client.id, client]));
    },
  },
